...
class SafeRandomSampler(torch.utils.data.sampler.Sampler):
    ...
    def __iter__(self):
        ...
        # Create indices on CPU first, then move if needed
        if torch.cuda.is_available():
            # Generate on CPU then move
            indices = torch.randperm(n, device='cpu')
...
def train():
    ...
    sampler = SafeRandomSampler(dataset)
